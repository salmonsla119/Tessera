...
| 로컬/AI 덱빌더의 보유 게이트 (변경됨) | 처음에는 로컬/AI 핫시트가 계정이 필요 없다는 이유로 게이트를 건너뛰게 했으나, 이후 요청으로 **모든 덱빌더(로컬·AI·온라인)가 항상 `fetchOwnership()`을 거치게** 바꿨다 — 로그인 상태면 계정의 실제 인벤토리, 아니면 시작 지급 4+4종만 보유한 것으로 취급한다 | 계정 로그인이 모드와 무관하게 "무엇을 편성할 수 있는가"를 결정하는 유일한 기준이 되어, 로컬에서 가챠로 뽑은 콘텐츠를 온라인과 동일하게 바로 쓸 수 있다. 로그인하지 않아도 시작 지급분으로는 계속 플레이 가능 — `web/src/app.ts` `fetchOwnership()`, `web/src/ui/deckbuilder.ts` |
