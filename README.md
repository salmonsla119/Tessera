...
| 지형 생성과 배치 구역 | 지형 덩어리 생성 시 **배치 구역(rank 1~2, 7~8) 칸은 후보에서 제외** | 제외하지 않으면 한쪽이 매치 시작부터 늪지 위에 배치되는 등 선제 불이익이 생김 — `rules/src/terrain.ts` `generateTerrain()` |
...
| 지형 3종 동시 등장 시 크기 배분 | **먼저 자리 잡는 지형이 우선** — 전역 점유 칸을 공유해 서로 겹치지 않게 하고, 남은 칸이 부족한 지형은 목표(13~30칸)보다 작게, 자리가 없으면 등장 자체를 건너뛴다 | 배치 구역을 뺀 보드 여유 칸이 32칸뿐이라 3종×30칸(=90칸)을 동시에 못 채우는 게 산수상 당연함 — `rules/src/terrain.ts` `generateTerrain()` |
...
```
pnpm test    →  95 tests passed   (rules 87 + server 8)
```
