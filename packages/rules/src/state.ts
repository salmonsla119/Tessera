import { EVA_CAP, SECOND_PLAYER_BONUS_TENTHS, requireBase } from '@tessera/data';
...
/**
 * 실효 회피율. 상태이상 보정을 모두 더한 뒤 0~60%로 자른다 (GDD §3.3).
 * 상한은 최종값에 적용되므로 EVA 80 기물은 상태이상이 없으면 60%가 된다.
 */
export function effectiveEva(piece: PieceState): number {
